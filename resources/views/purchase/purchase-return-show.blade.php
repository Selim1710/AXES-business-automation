    ...
    <div class="card mb-4">
                            ...
                            <div class="modal-body">
                                <div class="border p-3 rounded">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" class="form-control" name="vendor" placeholder="e.g Md. Sohel Enterprise" required>
                                    </div>
                                    <div class="col-12">
        ...
        <div class="card-body">
            <table id="datatablesSimple">
                ...
                @php $i=1 @endphp
                @foreach($PurchaseReturn as $PurchaseReturns)
                    <tr>
                        <td>{{ $i++ }} </td>
                        <td>{{ $PurchaseReturns->date }} </td>
                        <td>{{ $PurchaseReturns->vendor }} </td>
                        <td>{{ $PurchaseReturns->invoice }} </td>
                        <td>{{ $PurchaseReturns->total }} </td>
                        <td>{{ $PurchaseReturns->note }} </td>



                        <td>
                            <div style="min-width: 10rem;">
                                <a class="btn btn-success" style="font-size:13px" href="{{route('purchase-return-edit',['id'=>$PurchaseReturns->id])}}" role="button"><i class="fa fa-pencil" aria-hidden="true"></i> </a>
                                <form action="{{route('purchase-return-delete')}}" method="post" style="display:inline">
                                    @csrf
                                    <input type="hidden" name="purchase_return_delete" value="{{$PurchaseReturns->id}}">
